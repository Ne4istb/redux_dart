  ...
  static void run() {
    group('Redux store', () {
      ...
      test('Should reset middleware executing state if midleware reject chain', () async {
        ...
        Middleware rejectingMiddleware = (store) => (next) => (action) async {
      ...
      'dispatching', () async {
        ...
        store.dispatch(separatorAction);
        await await store.dispatch(addRecordAction('Line 2'));
      ...
      test('Should execute next invoke in the current execution'
      'dispatching', () async {
        var separatorAction = addRecordAction('Separator');
        Middleware middleware = (store) => (next) => (action) {
          if (action == separatorAction) {
            next(addRecordAction('One more'));
          }
